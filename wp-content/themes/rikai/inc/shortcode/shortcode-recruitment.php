 ...
 * @package Uni_Theme
 */
class uni_recruitment_shortcode {

	public static $args;

	public function __construct() {

		add_shortcode( 'unirecruitment', array( $this, 'render' ) );
	...
	function render( $atts, $content = '') {
		...
		extract( shortcode_atts( array(
			...
			'posts_per_page'				=> '5',
			'categories'					=> '',
			'viewmore_text'					=> __( 'See details', 'shtheme' ),
			'post_category'					=> '1',
			'post_link'					    => '1',
			'post_meta'						=> '1',
			'post_thumb'					=> '1',
	...
	function general_post_html ( $post_class = array(), $atts = array(), $image_size = 'large' ) {
		...
		$html .= '<div class="post-info__content">';
			...
			if ( $post_desc == '1' ) {
				...
				$quantity = !empty(get_field('quantity',get_the_ID()))?get_field('quantity',get_the_ID()):'Updating';
				$location = !empty(get_field('location-re',get_the_ID()))?get_field('location-re',get_the_ID()):'Updating';
				$date = !empty(get_field('date-end',get_the_ID()))?get_field('date-end',get_the_ID()):'Updating';
				// $location = get_field('location',get_the_ID());
				$html .= '<div class="post-info__description">';
					if($excerpt) {
						$html .= '<ul class="excerpt-list">';
					...
					// if($quantity) {
						$html .= '<p class="quantity">'.__('Quantity','shtheme').': '.$quantity.'</p>';
					// }
					// if($location) {
						$html .= '<p class="quantity">'.__('Location','shtheme').': '.$location.'</p>';
						$html .= '<p class="quantity">'.__('End date','shtheme').': '.$date.'</p>';
					// }
				$html .= '</div>';
			}
			// Check display view more button
			if ( $post_link == '1' ) {
				$html .= '<div class="post-info__link"><a href="'. get_permalink() .'" title="'. get_the_title() .'">'. $viewmore_text .' <img src="'.get_stylesheet_directory_uri().'/assets/img/icon/icon-right.svg" alt="Right"></a></div>';
			}
		$html .= '</div>';
		$html .= '</div></article>';
		return $html;
