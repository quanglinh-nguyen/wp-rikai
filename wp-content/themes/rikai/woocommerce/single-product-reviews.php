<?php
/**
 * Display single product reviews (comments)
 *
 * This template can be overridden by copying it to yourtheme/woocommerce/single-product-reviews.php.
 *
 * HOWEVER, on occasion WooCommerce will need to update template files and you
 * (the theme developer) will need to copy the new files to your theme to
 * maintain compatibility. We try to do this as little as possible, but it does
 * happen. When this occurs the version of the template file will be bumped and
 * the readme will list any important changes.
 *
 * @see 	    https://docs.woocommerce.com/document/template-structure/
 * @package 	WooCommerce/Templates
 * @version 4.3.0
 */

defined( 'ABSPATH' ) || exit;

global $product;

if ( ! comments_open() ) {
	return;
}

$review_ratings_enabled =  wc_review_ratings_enabled();
?>
<div id="reviews" class="woocommerce-Reviews row">
	<div id="comments" class="col-12">
		<h2 class="single-product-title normal">
			<?php
			$count = $product->get_review_count();
			if ( $count && wc_review_ratings_enabled() ) {
				/* translators: 1: reviews count 2: product name */
				$reviews_title = sprintf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'woocommerce' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
				echo apply_filters( 'woocommerce_reviews_title', $reviews_title, $count, $product ); // WPCS: XSS ok.
			} else {
				echo '<span>'.esc_html_e( 'Reviews', 'woocommerce' ).'</span>';
			}
			?>
		</h2>

        <?php 
            $rating_count 	= $product->get_rating_count(get_the_ID());
            $review_count 	= $product->get_review_count(get_the_ID());
            $average      	= round($product->get_average_rating(get_the_ID()),1);
			$percent		= $average/5*100;
            ?>
            <div class="star_box">
                <div class="star-average d-flex flex-column justify-content-center align-items-center mb-md-0 mb-2">
                    <span class="star_average">
                        <?php echo $average; ?>/5
                    </span>
					<div class="star-progress">
						<?php 
							echo '<div class="list-star">';
								for ($i=0; $i < 5; $i++) { 
									echo '<i class="icon-star-empty"></i>';
								}
								echo '<div class="fill-stars" style="width:'.$percent.'%">';
									for ($i=0; $i < 5; $i++) { 
										echo '<i class="icon-star"></i>';
									}
								echo '</div>';
							echo '</div>';
						?>
					</div>
					
					<?php 
						if ( 0 === $rating_count ) {
							echo '<div class="rating_count text-center">' . __( 'There are no reviews yet.', 'woocommerce' ) . '</div>';
						} elseif ( 1 === $rating_count ) {
							echo '<div class="rating_count text-center">' . $rating_count .'&nbsp;'. __( 'Reviews', 'shtheme' ) . '</div>';
						} else {
							echo '<div class="rating_count text-center">' . $rating_count .'&nbsp;'. __( 'Reviews', 'shtheme' ) . '</div>';
						}
					?>
                </div>
                <div class="reviews-star__boxleft">
                    <div class="reviews-starbar__wrap">
                        <?php 
                            for ($i = 5; $i >= 1; $i--) { 
                                echo '<div class="reviews-starbar__row">';
                                    echo rating_percent($i);
                                echo '</div>';
                            }
                        ?>
                    </div>
                </div>
                <div class="reviews-star__boxright">
                    <a href="javascript:void(0)" title="Đánh giá ngay" class="btn-reviews-now">Đánh giá ngay</a>
                </div>
            </div>
            <?php
        ?>

		<?php 
            if ( have_comments() ) : 
            ?>

			<ol class="commentlist">
				<?php wp_list_comments( apply_filters( 'woocommerce_product_review_list_args', array( 'callback' => 'woocommerce_comments' ) ) ); ?>
			</ol>

			<?php
			if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
				echo '<nav class="woocommerce-pagination">';
				$pagination = paginate_comments_links(
					apply_filters(
						'woocommerce_comment_pagination_args',
						array(
							'prev_text' => '&larr;',
							'next_text' => '&rarr;',
							'type'      => 'list',
							'echo'      => false,
						)
					)
				);
				$pagination = str_replace( 'page-numbers', 'page-number', $pagination );
				$pagination = str_replace( "<ul class='page-number'>", '<ul class="page-numbers nav-pagination links text-center">', $pagination );
				echo $pagination;
				echo '</nav>';
			endif;
		?>
		<?php endif; ?>
	</div>

	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
		<div id="uni-review" class="col-12">
            <div class="uni-overlay"></div>
			<div id="uni-review__form" class="col-inner">
				<button type="button" class="uni-close">×</button>
				<div class="uni-review__info has-border">
				<?php
				$commenter    = wp_get_current_commenter();
				$comment_form = array(
					/* translators: %s is product title */
					'title_reply'          => have_comments() ? esc_html__( 'Add a review', 'woocommerce' ).' '.get_the_title() : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'woocommerce' ), get_the_title() ),
					/* translators: %s is product title */
					'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'woocommerce' ),
					'title_reply_before'   => '<h3 id="reply-title" class="comment-reply-title">',
					'title_reply_after'    => '</h3>',
					'comment_notes_before' => '',
					'comment_notes_after'  => '',
					'label_submit'         => esc_html__( 'Review', 'shtheme' ),
					'logged_in_as'         => '',
					'comment_field'        => '',
				);

				$name_email_required = (bool) get_option( 'require_name_email', 1 );

				$fields              = array(
					'author' => array(
						'label'    => __( 'Name', 'woocommerce' ),
						'type'     => 'text',
						'value'    => $commenter['comment_author'],
						'required' => $name_email_required,
					),
					'email'  => array(
						'label'    => __( 'Email', 'woocommerce' ),
						'type'     => 'email',
						'value'    => $commenter['comment_author_email'],
						'required' => $name_email_required,
					),
				);

				$comment_form['fields'] = array();

				foreach ( $fields as $key => $field ) {
					$field_html  = '<p class="comment-form-' . esc_attr( $key ) . '">';
					$field_html .= '<label for="' . esc_attr( $key ) . '">' . esc_html( $field['label'] );

					if ( $field['required'] ) {
						$field_html .= '&nbsp;<span class="required">*</span>';
					}

					$field_html .= '</label><input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" type="' . esc_attr( $field['type'] ) . '" value="' . esc_attr( $field['value'] ) . '" size="30" ' . ( $field['required'] ? 'required' : '' ) . ' /></p>';

					$comment_form['fields'][ $key ] = $field_html;
				}

				$account_page_url = wc_get_page_permalink( 'myaccount' );
				if ( $account_page_url ) {
					/* translators: %s opening and closing link tags respectively */
					$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( esc_html__( 'You must be %1$slogged in%2$s to post a review.', 'woocommerce' ), '<a href="' . esc_url( $account_page_url ) . '">', '</a>' ) . '</p>';
				}
				if ( $review_ratings_enabled ) {
					$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'woocommerce' ) . ( wc_review_ratings_required() ? '&nbsp;<span class="required">*</span>' : '' ) . '</label>';
					$comment_form['comment_field'] .= '<div class="box-form-vote">
					        <label class="rate" data-number="1">
					            <i class="icon-star one-star"></i>
					            <span>' . esc_html__( 'Very poor', 'woocommerce' ) . '</span>
					        </label>
					        <label class="rate" data-number="2">
					            <i class="icon-star two-star"></i>
					            <span>' . esc_html__( 'Not that bad', 'woocommerce' ) . '</span>
					        </label>
					        <label class="rate" data-number="3">
					            <i class="icon-star three-star"></i>
					            <span>' . esc_html__( 'Average', 'woocommerce' ) . '</span>
					        </label>
					        <label class="rate" data-number="4">
					            <i class="icon-star four-star"></i>
					            <span>' . esc_html__( 'Good', 'woocommerce' ) . '</span>
					        </label>
					        <label class="rate" data-number="5">
					            <i class="icon-star five-star"></i>
					            <span>' . esc_html__( 'Perfect', 'woocommerce' ) . '</span>
					        </label>
					</div>';
					$comment_form['comment_field'] .= '<select name="rating" id="rating" required>
						<option value="">' . esc_html__( 'Rate&hellip;', 'woocommerce' ) . '</option>
						<option value="5">' . esc_html__( 'Perfect', 'woocommerce' ) . '</option>
						<option value="4">' . esc_html__( 'Good', 'woocommerce' ) . '</option>
						<option value="3">' . esc_html__( 'Average', 'woocommerce' ) . '</option>
						<option value="2">' . esc_html__( 'Not that bad', 'woocommerce' ) . '</option>
						<option value="1">' . esc_html__( 'Very poor', 'woocommerce' ) . '</option>
					</select></div>';
				}

				$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . esc_html__( 'Your review', 'woocommerce' ) . '&nbsp;<span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" required></textarea></p>';

				comment_form( apply_filters( 'woocommerce_product_review_comment_form_args', $comment_form ) );
				?>
				</div>
			</div>
		</div>

	<?php else : ?>
		<div id="uni-review" class="large-<?php if ( get_comment_pages_count() == 0 ) { echo '12'; } else { echo '5'; } ?> col">
			<div id="uni-review__form" class="col-inner">
				<button type="button" class="close">×</button>
				<div class="uni-review__info has-border">
					<p class="woocommerce-verification-required"><?php esc_html_e( 'Only logged in customers who have purchased this product may leave a review.', 'woocommerce' ); ?></p>
				</div>
			</div>
		</div>
	<?php endif; ?>

	<script type="text/javascript">
		jQuery(document).ready(function($) {
			const REVIEWS = {
				rating() {
					jQuery('body').on('click', '.rate', function() {
						jQuery(this).find('.icon-star').addClass('checked');
						jQuery(this).prevAll().find('.icon-star').addClass('checked');
						jQuery(this).nextAll().find('.icon-star').removeClass('checked');
						jQuery(this).closest("#respond").find("#rating").val(jQuery(this).data('number'));
					});
				},
				init() {
					this.rating();
				}
			}
			REVIEWS.init();
		});
	</script>

</div>
