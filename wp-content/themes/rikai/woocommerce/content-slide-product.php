...
<li <?php wc_product_class( $classes, $product ); ?>>
	<div class="product-info">
		...
		echo '<div class="product-info__thumbnail">';
			echo '<a class="img hover-zoom" href="'. get_permalink( ) .'" title="'. get_the_title( ) .'">';
				echo ' <img data-lazy="'.get_the_post_thumbnail_url( get_the_ID(),'large').'" alt="'. get_the_title( ) .'"> ';
                do_action( 'woocommerce_shop_loop_item_image' );
            echo '</a>';
            do_action( 'woocommerce_shop_button_action' );
		echo '</div>';
		
		echo '<div class="product-info__content">';
